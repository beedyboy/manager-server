...
router.get("/:id", (req, res) => {
    ...
    db('conversations').where({ticket_id}).select().then( ( data ) => { 
     if(data) {
         res.send({
             status: 200,
             data
...
router.post("/", checkHeader, (req, res, next) => {   
   try {
    ...
    db('conversations').insert({  ticket_id, description, respondent, created_at }).then( ( result ) => { 
...
router.post("/update", checkHeader, (req, res) => {  
  ...
  db('conversations').where('id', id).update( { ticket_id, description, respondent,  updated_at })
  .then( ( data ) => {  
    if(data) {
      res.send({
      status: 200, 
...
router.delete("/:id", checkHeader, (req, res) => { 
   try { 
    db('conversations').where('id', req.params.id).del().then( (result) => {
        res.send({
            status: 200,
