...
updateStatus: async (tbl, id, status) => {
 ...
 await db(tbl).where({id}).update({ status, updated_at}).then( ( data ) => { 
   if(data) {  
         reply = true;
     } else {reply = false;}
...
minusStock: async (id, sold) => {
  var reply = false;
  const updated_at = new Date().toLocaleString();
  await db('stocks').where({id}).select().then((stock) => {
    let oldQty = stock[0].quantity;
    const quantity = oldQty - sold;
    await db('stocks').where({id}).update({ quantity, updated_at}).then( ( data ) => { 
      if(data) {  
            reply = true;
        } else {reply = false;}
